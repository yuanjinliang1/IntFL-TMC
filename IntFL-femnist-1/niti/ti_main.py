...
def main():
    ...
    clients = setup_clients(model, data=None, optimizer=optimizer)

    plot_result = []

    for epoch in range(args.start_epoch, args.num_round):
        print('current round: ' + str(epoch))
        ...
        # train
        print('---------train----------')
        server.train_model(epoch, args.num_epochs, 20, server.selected_clients, args.model_type)
        # aggregate
        print('---------aggreagate----------')
        if args.model_type =='int':
            server.update_model()
        else:
            server.update_float_model()
        ...
        if epoch % 200 == 0:
            save_file_name = 'result-niti-int-mnist-' + str(args.num_clients) + '-round' + str(epoch) + '-localEpoch' + str(args.num_epochs) +'-epoch.npy'
            np.save(save_file_name, plot_result)
    save_file_name = 'result-niti-int-mnist-' + str(args.num_clients) + '-round' + str(args.num_round) + '-localEpoch' + str(args.num_epochs) +'-epoch.npy'
    np.save(save_file_name, plot_result)


def setup_clients(model=None, data=None, optimizer=None):
