...
def main():
    ...
    for epoch in range(args.start_epoch, args.num_round):
        ...
        if epoch % args.log_interval == 0:
            ...
            print('current test round: ' + str(epoch))
            print('average accuracy: ' + str(avg_acc))
            # logging.info('select-clients {:d} '
            #                 'local-epoch {:d} \n'
            #                 .format(args.num_clients, args.num_epochs))
            # logging.info('current round {:d} '
            #                 'accuracy {:.3f} \n'
            #                 .format(epoch, avg_acc))
        if epoch % 200 == 0 :
            save_file_name = 'output-test-1/result-niti-hybrid-update-femnist-' + str(args.num_clients) + '-round' + str(epoch) + '-gb' + str(args.grad_bitwidth) + '-epoch' + str(args.num_epochs) + '.npy'
            np.save(save_file_name, plot_result)
    
    save_file_name = 'output-test-1/result-niti-hybrid-update-femnist-' + str(args.num_clients) + '-round' + str(args.num_round) + '-gb' + str(args.grad_bitwidth) + '-epoch' + str(args.num_epochs) + '.npy'
    np.save(save_file_name, plot_result)
...
def setup_clients(model_type='int'):
    ...
    if model_type == 'hybrid':
        ...
        clients = create_clients(users, train_data, test_data, model, optimizer)
...
def create_clients(users, train_data, test_data, model, optimizer):
    clients = [Client(u, train_data[u], test_data[u], model, optimizer) for u in users]
    return clients
...
def create_hybrid_clients(users, train_data, test_data, model_types):
    ...
    for i, model_type in enumerate(model_types):
        ...
        clients.append(Client(users[i], train_data[users[i]], test_data[users[i]]))
    return clients
